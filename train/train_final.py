...
class Trainer():
        ...
        for name, m in self.model.named_modules():
            ...
            if hasattr(m, 'weight'):
                if m.weight is not None and name in self.state_weight:
                    ...
                    self._stat_code(dict, reconstruction, code, quant, stat_mode)
    ...
    def _stat_code(self, dict, reconstruction, code, quant, stat_mode):
        if stat_mode == 'test':
            np_quant = to_np(reconstruction)
...
def main1(args):
    from data_new.data import Imagenet
    ...
    # testloader = Imagenet().val_dataloader()
