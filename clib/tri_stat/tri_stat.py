...
stat = load(name="stat",
            sources=[os.path.join(os.path.split(os.path.abspath(__file__))[0], "tri_stat.cpp"),
                     os.path.join(os.path.split(os.path.abspath(__file__))[0], "tri_stat.cu")])
print("stat module loaded")
class TriearStatFun(torch.autograd.Function):
    @staticmethod
...
class Triear_Stat(torch.nn.Module):
    ...
    def forward(self, matrix, queries):
        ...
        rescdf_i, rescdf_f = TriearStatFun.apply(matrix_flatten, queries_flatten, delta)
        return rescdf_i.view(queries.shape), rescdf_f.view(queries.shape)

if __name__ == '__main__':
    import argparse

    tmp = (torch.arange(10, dtype=torch.float)).requires_grad_(True)
    test = (torch.tensor([5.1])).requires_grad_(True)
    rescdf_i, rescdf_f = TriearStatFun.apply(tmp, test, 1.0)
