...
stat = load(name="stat",
          sources=[os.path.join(os.path.split(os.path.abspath(__file__))[0], "lin_stat.cpp"),
                   os.path.join(os.path.split(os.path.abspath(__file__))[0], "lin_stat.cu")])
print("stat module loaded")
class LinearStatFun(torch.autograd.Function):
    @staticmethod
...
class Linear_Stat(torch.nn.Module):
    ...
    def forward(self, matrix, queries):
        ...
        rescdf_i, rescdf_f = LinearStatFun.apply(matrix_flatten, queries_flatten, delta)
...
if __name__ == '__main__':
    ...
    rescdf_i, rescdf_f = LinearStatFun.apply(tmp, test, 1.0/2)
