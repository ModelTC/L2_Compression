...
__global__ 
void LinStatKernel(const int m , const int n, const float* sorted_matrix_data, const float* queries_data, const float delta, int* rescdf_i, float* rescdf_f, float* respdf_f)
{
        ...
        for (int i=threadIdx.x+blockIdx.y*blockDim.x+blockIdx.x*blockDim.x*gridDim.y;i<n;i+=blockDim.x*gridDim.y*gridDim.x) {
            ...
            for (int j = start; j < end_k; j++) {
                if (query_data_i > buf[j] + delta) {
                    ...
                    rescdf_f_i += ((query_data_i - buf[j] + delta) / delta) / 2;
                    respdf_f_i += (1 /  delta) / 2;
                }
            }
...
__global__
void LinStatGradKernel(const int m , const int n, const float* matrix_data, const float* queries_data, const float delta, const float* grad_cdf_f, float* res_grad_matrix_f)
{    
    const int batch=512;
...
{
	LinStatKernel<<<dim3(32,16,1),512>>>(m, n, sorted_matrix_data, queries_data, delta, rescdf_i, rescdf_f, respdf_f);

// 	cudaError_t err = cudaGetLastError();
// 	if (err != cudaSuccess)
// 	    printf("error in linstat Output: %s\n", cudaGetErrorString(err));
}
...
{
	LinStatGradKernel<<<dim3(32,16,1),512>>>(m, n, matrix_data, queries_data, delta, grad_cdf_f, res_grad_matrix_f);
